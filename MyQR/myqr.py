...
def run(words, version=1, level='H', picture=None, colorized=False, contrast=1.0, brightness=1.0, save_name=None, save_dir=os.getcwd()):
    ...
    def combine(ver, qr, bg_name, colorized, contrast, brightness, image_format, save_dir, save_name=None):
    ...
    try:
        # Make directory that to save QRcode.
        if not os.path.exists(tempdir):
            os.makedirs(tempdir)

        # Make QRcode
        ver, qr = theqrmodule.get_qrcode(version, level, words, tempdir) 

        if picture and picture.format=='GIF':   # If gif, combine many image
            import imageio
            ...
            while True:
                try:
                    ...
                    temp_io = BytesIO()
                    im.save(temp_io,'PNG')
                    im_io.append(temp_io)                              # SAVE IN BYTES IO
            ...
            imsname = []
            for io in im_io:
                image_io = Image.open(io)
                # bg_name = os.path.join(tempdir, '%s.png' % s)
                imsname.append(combine(ver, qr, image_io, colorized, contrast, brightness, 'GIF', tempdir))
            
            ims = [pic for pic in imsname]
        ...
        elif picture:  # If image, combine one image
            qr_image = combine(ver, qr, picture, colorized, contrast, brightness, 'PNG', save_dir, save_name)
            qr_io = BytesIO()
            qr_image.save(qr_io,'PNG')
            qr_io.seek(0)
        elif qr: # If not exists input image
